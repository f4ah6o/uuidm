...
test "pbt_round_trip_string_with_coverage" {
  ...
  for i = 0; i < 200; i = i + 1 {
    ...
    if not(uuid.is_nil()) && not(uuid.is_max()) {
...
test "pbt_uuid_string_length_coverage" {
  ...
  for i = 0; i < 200; i = i + 1 {
    ...
    if get_version_char(uuid_str) == '4' {
      v4_count = v4_count + 1
    }
    let str = uuid.to_string()
...
test "pbt_version_distribution" {
  ...
  for uuid in uuids {
    ...
    assert_true(
      variant_is_rfc9562(uuid),
      msg="UUID should have RFC 9562 variant",
    )
  }
...
test "pbt_name_based_uuid_diversity" {
  ...
  let mut normal_count = 0
  let test_names = ["", "测试", "test", "example.com"]
...
test "pbt_bytes_reversibility_coverage" {
  ...
  for bytes in byte_arrays {
    if bytes[0] == b'\x00' && bytes[1] == b'\x00' {
      zero_count = zero_count + 1
    }
    if bytes[0] == b'\xFF' && bytes[1] == b'\xFF' {
      max_count = max_count + 1
    }
...
test "pbt_v4_uuids_well_formed" {
  ...
  for i = 0; i < 100; i = i + 1 {
    let uuid = v4()
    let uuid_str = uuid.to_string()
    if variant_is_rfc9562(uuid) {
      rfc9562_count = rfc9562_count + 1
    }
